...
The wrapper is a thin stub that does `@import url(... raw GitHub ...)` of the real CSS. Every time you launch Discord (or hit Ctrl+R) it pulls the freshest version. Trade-off: needs internet on launch (same as the Google Fonts `@import` already does), and you can't pin to a specific version.
...
Обёртка просто делает `@import url(... raw GitHub ...)` основного CSS. На каждом старте Discord (или Ctrl+R) подтягивается свежая версия. Минус: нужен интернет при старте (так же, как для `@import` Google Fonts), и нельзя зафиксироваться на конкретной версии.
